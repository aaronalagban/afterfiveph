...
function SidebarLink({ label, sub, active, onClick, href, color, icon, darkMode, highlight = false, feteTheme = false }: SidebarLinkProps) {
  ...
  if (href) {
    return (
      <Link href={href} className={`block w-full p-6 text-left border-b transition-colors duration-500 group relative overflow-hidden flex items-center justify-between ${active ? activeBg : baseBg}`}>
        {content}
      </Link>
    );
  }
  ...
  return (
    <button onClick={onClick} className={`w-full p-6 text-left border-b transition-colors duration-500 group relative overflow-hidden flex items-center justify-between ${active ? activeBg : baseBg}`}>
      {content}
    </button>
  );
}
...
function MobileNavBtn({ icon, active, onClick, href, color, darkMode, highlight = false, feteTheme = false }: MobileNavBtnProps) {
  ...
  if (href) {
    return (
      <Link href={href} className={`flex-1 flex items-center justify-center transition-colors duration-500 ${active ? activeBg : baseBg}`}>
        {content}
      </Link>
    );
  }
  ...
  return (
    <button onClick={onClick} className={`flex-1 flex items-center justify-center transition-colors duration-500 ${active ? activeBg : baseBg}`}>
      {content}
    </button>
  );
}
